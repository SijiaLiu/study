...
public class Heap {
    ...
    public static void heapDown(List<Integer> heap, int index) {
        ...
        // 2*index>n说明该节点没有左右儿子节点了，判断一下当前节点的的根节点父节点是否满足大顶堆的性质，因为有可能最后一个的元素比删除节点的父节点还大
        if (2 * index > n) {
            heapDown(heap, index / 2);
            return;
        } // 如果左右儿子都存在 因为第一个位置存储的是空值，不在考虑之内，n+1不会越界
        else if (2 * index <= n) {
            // 定义左儿子节点
            child = 2 * index;
    ...
    public static void main(String args[]) {
        List<Integer> array = new ArrayList<>(Arrays.asList(null, 40, 20, 38, 18, 16, 25));
        adjust(array);// 调整使array成为最大堆

        delete(array, 5);// 堆中删除下标是8的元素
